...
{
    uint32_t integer_part;  // 整数部分，使用更大的数据类型
    uint32_t decimal_part;  // 小数部分，使用更大的数据类型
    int8_t sign = 1;        // 符号位(1:正, -1:负)
    uint8_t char_width;     // 字符宽度
    
    // 计算字符宽度
    char_width = (sizey == 8) ? 6 : 12;  // 假设8号字体宽6像素，16号字体宽12像素
    ...
    decimal_part = (uint32_t)((num - integer_part) * multiplier);
    ...
    // 显示整数部分
    OLED_ShowNum(x, y, integer_part, int_len, sizey);
    x += int_len * char_width;  // 移动到小数点位置
    
    // 显示小数点
    if (dec_len > 0)  // 只有当需要显示小数部分时才显示小数点
    {
        OLED_ShowChar(x, y, '.', sizey);
        x += char_width / 2;  // 小数点宽度通常是字符宽度的一半
        
        // 显示小数部分
        OLED_ShowNum(x, y, decimal_part, dec_len, sizey);
    }
}
